const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["site-chunks/monaco-CH-VS9lI.js","site-app.js","site-chunks/monaco-BJ215w34.css"])))=>i.map(i=>d[i]);
import{s as e,t}from"../site-app.js";var n=e=>e.replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function r(e,t,n){for(let i=n+t.length;i<e.length;){if(e[i]===`\\`){i+=2;continue}if(t!=="`"&&e[i]==="`"){let t=r(e,"`",i);if(t<0)return-1;i=t+1;continue}if(e.startsWith(t,i)&&(t!==`*`||e[i-1]!==`*`&&e[i+1]!==`*`))return i;i++}return-1}function i(e){let t=``,a=``,o=()=>{a&&=(t+=n(a),``)};for(let s=0;s<e.length;){if(e[s]===`\\`&&s+1<e.length){a+=e[s+1],s+=2;continue}if(e[s]==="`"){let i=r(e,"`",s);if(i>s+1){o(),t+=`<code>${n(e.slice(s+1,i))}</code>`,s=i+1;continue}}if(e[s]===`[`){let r=e.indexOf(`](`,s+1),a=r>=0?e.indexOf(`)`,r+2):-1;if(r>s+1&&a>r+2){let c=e.slice(r+2,a);if(/^https?:\/\/[^\s]+$/i.test(c)){o(),t+=`<a href="${n(c)}" target="_blank" rel="noopener noreferrer">${i(e.slice(s+1,r))}</a>`,s=a+1;continue}}}let c=e.startsWith(`**`,s)?`**`:e.startsWith(`~~`,s)?`~~`:e[s]===`*`?`*`:``;if(c){let n=r(e,c,s);if(n>s+c.length){o();let r=i(e.slice(s+c.length,n));t+=c===`**`?`<strong>${r}</strong>`:c===`~~`?`<del>${r}</del>`:`<em>${r}</em>`,s=n+c.length;continue}}a+=e[s++]}return o(),t}function a(e){let t=e.replace(/\r/g,``).split(`
`),r=[],a=0,o=(e,t,n)=>`<div data-source-start="${e}" data-source-end="${t}">${n}</div>`;for(;a<t.length;){let e=a,s=t[a];if(s.startsWith("```")){let i=s.slice(3).trim(),c=[];for(a++;a<t.length&&!t[a].startsWith("```");)c.push(t[a++]);a<t.length&&a++,r.push(o(e,a,`<pre${i?` data-language="${n(i)}"`:``}><code>${n(c.join(`
...
- Live local preview
- Source-aware linked scrolling`),s=l(`markdown`,`linked`,`1`)!==`0`,c=l(`markdown`,`view`,`combined`);[`combined`,`source`,`preview`].includes(c)||(c=`combined`),n.innerHTML=`<section class="markdown-tool" data-view="${c}"><div class="markdown-source"><div id="md-input" class="monaco-host"></div></div><article id="md-output" class="markdown-preview"></article></section>`;let p=t.editor.createModel(o,`markdown`),h=t.editor.create(n.querySelector(`#md-input`),T(`markdown`));h.setModel(p);let _=n.querySelector(`#md-output`),v=!1,b=[],S=()=>{let e=_.getBoundingClientRect();b=[..._.querySelectorAll(`[data-source-start]`)].map(t=>{let n=t.getBoundingClientRect();return{start:+t.dataset.sourceStart,end:+t.dataset.sourceEnd,top:n.top-e.top+_.scrollTop,bottom:n.bottom-e.top+_.scrollTop}}).filter(e=>Number.isFinite(e.start)&&Number.isFinite(e.end))},C=e=>{let t=b.find(t=>e>=t.start&&e<=t.end);return t?t.top+(e-t.start)/Math.max(1,t.end-t.start)*Math.max(1,t.bottom-t.top):b.find(t=>t.start>=e)?.top??_.scrollHeight},O=e=>{let t=b.find(t=>e>=t.top&&e<=t.bottom);if(t)return t.start+(e-t.top)/Math.max(1,t.bottom-t.top)*Math.max(1,t.end-t.start);let n=b[0];for(let t of b){if(t.top>e)break;n=t}return n?.end??0},k=()=>{let e=h.getVisibleRanges()[0]?.startLineNumber||1,n=h.getTopForLineNumber(e),r=e<p.getLineCount()?h.getTopForLineNumber(e+1):n+h.getOption(t.editor.EditorOption.lineHeight);return e-1+Math.max(0,Math.min(1,(h.getScrollTop()-n)/Math.max(1,r-n)))},A=()=>{!s||v||!b.length||c===`source`||(v=!0,_.scrollTop=Math.max(0,C(k())-24),requestAnimationFrame(()=>{v=!1}))},j=()=>{if(!s||v||!b.length||c===`preview`)return;v=!0;let e=O(_.scrollTop+24),n=Math.max(1,Math.min(p.getLineCount(),Math.floor(e)+1)),r=h.getTopForLineNumber(n),i=n<p.getLineCount()?h.getTopForLineNumber(n+1):r+h.getOption(t.editor.EditorOption.lineHeight);h.setScrollTop(r+(e-Math.floor(e))*Math.max(1,i-r),t.editor.ScrollType.Immediate),requestAnimationFrame(()=>{v=!1})},M=()=>`<span class="markdown-view-toggle" role="group" aria-label="Markdown view"><button type="button" data-md-view="combined" aria-pressed="${c===`combined`}">Combined</button><button type="button" data-md-view="source" aria-pressed="${c===`source`}">Source</button><button type="button" data-md-view="preview" aria-pressed="${c===`preview`}">Preview</button></span><button type="button" data-md-link aria-pressed="${s}" aria-label="Toggle linked scrolling" title="Toggle linked scrolling">${m(`link`)}</button>`,N=()=>{x(M()),r.querySelectorAll(`[data-md-view]`).forEach(e=>e.onclick=()=>{c=e.dataset.mdView,u(`markdown`,`view`,c),n.querySelector(`.markdown-tool`).dataset.view=c,N(),requestAnimationFrame(()=>{h.layout(),S(),s&&c===`combined`&&A()})}),r.querySelector(`[data-md-link]`).onclick=()=>{s=!s,u(`markdown`,`linked`,s?`1`:`0`),N(),s&&A()}},P=()=>{let e=p.getValue();f(`text`,e),_.innerHTML=a(e),S(),N(),s&&A()},F=p.onDidChangeContent(P),I=h.onDidScrollChange(e=>{e.scrollTopChanged&&A()});_.addEventListener(`scroll`,j,{passive:!0}),P(),g=()=>{F.dispose(),I.dispose(),_.removeEventListener(`scroll`,j),h.dispose(),p.dispose()}}function ue(){let t=++v;try{g()}catch{}g=()=>{};let r=i(),a={text:A,base:ne,diff:ie,number:ce,markdown:le}[r]||A;document.title=`${e.find(({id:e})=>e===r)?.title||`Tools`} · Sanyam Brar`,Promise.resolve(a(t)).catch(e=>{D(t)&&(n.innerHTML=`<div class="tool-fatal"><strong>Editor failed to load.</strong><span>${p(e?.message||String(e))}</span><button type="button">Retry</button></div>`,n.querySelector(`button`).onclick=ue,x(``))})}return ue(),()=>{_=!0,v++;try{g()}catch{}x(``),b&&removeEventListener(`samey-themechange`,b)}}export{l as mountTool};