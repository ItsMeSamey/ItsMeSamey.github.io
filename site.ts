...
const ext=(href:string)=>/^https?:/.test(href)?' rel="noopener noreferrer"':'';
...
const appearanceButton=`<button class="top-icon" type="button" data-samey-appearance aria-label="Appearance" aria-expanded="false"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M4.93 4.93l2.12 2.12M16.95 16.95l2.12 2.12M2 12h3M19 12h3M4.93 19.07l2.12-2.12M16.95 7.05l2.12-2.12"/></svg></button>`;
const header=(active:string)=>`<header class="top"><a class="brand" href="./">Sanyam Brar</a><nav class="top-nav" aria-label="Primary">${nav.map(([label,href])=>`<a href="${href}"${label.toLowerCase()===active?' aria-current="page"':''}>${label}</a>`).join('')}${appearanceButton}<button class="search-trigger" type="button" data-open-search aria-label="Search"><kbd data-search-shortcut>Ctrl K</kbd></button></nav></header>`;
const intro=()=>`<section class="intro" aria-label="About"><div class="intro-meta"><span>Zig</span><span>C++</span><span>Go</span><span>Java</span></div><div class="intro-links"><a href="https://github.com/ItsMeSamey" data-copy-label="Sanyam Brar on GitHub" rel="noopener noreferrer">GitHub ↗</a><a href="https://github.com/SmallThingz" data-copy-label="SmallThingz on GitHub" rel="noopener noreferrer">SmallThingz ↗</a></div></section>`;
...
const gameTitle=(i:number)=>i===0
  ...
  : i===1
    ? `<span class="game-title game-keybr" aria-label="Keybr"><i class="keybr-hit">K</i><i class="keybr-hit">e</i><i class="keybr-miss">y</i><i class="keybr-hit">b</i><i class="keybr-miss-bg">r</i><b class="keybr-cursor" aria-hidden="true"></b></span>`
    : `<span class="game-title game-chain" aria-label="Chain Reaction"><span>CHAIN</span><svg viewBox="0 0 58 22" aria-hidden="true"><path class="chain-link" d="M6 11h12M40 11h12"/><rect class="chain-cell chain-cell-a" x="2" y="7" width="8" height="8" rx="1"/><rect class="chain-cell chain-cell-core" x="20" y="4" width="18" height="14" rx="2"/><path class="chain-burst" d="M29 1v3M29 18v3M17 11h3M38 11h3"/><rect class="chain-cell chain-cell-b" x="48" y="7" width="8" height="8" rx="1"/></svg><span>REACTION</span></span>`;
...
const githubReadme=(href:string)=>{ const u=new URL(href); if(u.hostname==='github.com'&&!u.hash)u.hash='readme'; return u.href; };
const browserEmbed=(entry:Entry)=>{ const href=githubReadme(entry.href); return `<section class="detail-browser-section" aria-label="${esc(entry.title)} preview"><div class="detail-browser" data-inline-browser data-url="${esc(href)}"><div class="detail-browser-controls" data-browser-controls aria-label="Embedded page controls"><button class="detail-browser-button" type="button" data-browser-external aria-label="Open in new tab" title="Open in new tab"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 3h7v7"></path><path d="M10 14L21 3"></path><path d="M21 14v7H3V3h7"></path></svg></button><span class="detail-browser-divider" aria-hidden="true"></span><button class="detail-browser-button" type="button" data-browser-fullscreen aria-label="Enter fullscreen" title="Fullscreen"><svg viewBox="0 0 24 24" aria-hidden="true" data-browser-fullscreen-icon><path d="M8 3H3v5"></path><path d="M16 3h5v5"></path><path d="M8 21H3v-5"></path><path d="M16 21h5v-5"></path></svg></button></div><iframe src="${esc(href)}" title="${esc(entry.title)}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe></div></section>`; };
const detail=(_slug:string,d:typeof details[string])=>{ const primary=d.links.find(x=>/^https?:/.test(x.href)); return head(`${d.title} · Sanyam Brar`,`project`)+header('work')+`<main class="detail"><a class="backline" href="./work">‹ Work</a><article class="project-detail"><p class="eyebrow">Project</p><h1>${esc(d.title)}</h1><p class="dek">${esc(d.dek)}</p><div class="fact-strip">${d.facts.map(x=>`<span>${esc(x)}</span>`).join('')}</div><section class="detail-copy"><h2>What it is</h2><p>${esc(d.body)}</p></section>${primary?browserEmbed(primary):''}<section class="detail-copy"><h2>Related</h2>${compact(d.links)}</section></article></main>`+foot(); };
