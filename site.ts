...
const gameCards=()=>`<div class="grid">${games.map((x,i)=>`<a class="card" href="${x.href}" data-copy-label="${esc(x.title)}"><div class="card-top"><strong>${esc(x.title)}</strong><span class="game-mark game-mark-${i}" aria-hidden="true">${i===0?'W O R D L E':i===1?'keybr_':'● ● ●'}</span><span class="arrow">›</span></div><div class="card-copy">${esc(x.note)}</div></a>`).join('')}</div>`;
