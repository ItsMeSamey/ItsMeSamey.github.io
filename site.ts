...
const header=(active:string)=>`<header class="top"><a class="brand" href="./">Sanyam Brar</a><nav class="top-nav" aria-label="Primary">${nav.map(([label,href])=>`<a href="${href}"${label.toLowerCase()===active?' aria-current="page"':''}>${label}</a>`).join('')}<button class="search-trigger" type="button" data-open-search aria-label="Search"><kbd data-search-shortcut>Ctrl K</kbd></button></nav></header>`;
