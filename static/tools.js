(() => {
  ...
  function numberTool() {
    ...
    root.innerHTML = `<section class="number-tool"><div class="number-hero"><label><span>Value</span><input id="number-input" spellcheck="false" value="${esc(text)}"></label><label><span>Source base</span><input id="source-base" type="number" min="2" max="62" value="${base}"></label><div class="radix-choices">${[2, 8, 10, 16].map(value => `<button type="button" data-base="${value}">${value}</button>`).join('')}</div><div class="number-error" id="number-error" hidden></div></div><section class="number-representations"><header><h1>Representations</h1><label>Custom base <input id="custom-base" type="number" min="2" max="62" value="${custom}"></label></header><div class="number-grid"></div><section class="bit-panel"><header><h2>Bits</h2><span id="bit-caption"></span></header><div id="bit-values" class="bit-values"></div></section></section></section>`;
