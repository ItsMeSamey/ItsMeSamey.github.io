...
export function ParagraphSelector({ paragraphs, paragraphIndex, onChange, }: {
    ...
    return (<FieldList>
      ...
      <Field>
        <Dir swap="icon">
          <IconButton icon={<Icon shape={mdiSkipPrevious}/>} disabled={paragraphIndex === 0} onClick={() => {
            if (paragraphIndex > 0) {
                onChange(paragraphIndex - 1);
            }
        }}/>
          <IconButton icon={<Icon shape={mdiSkipNext}/>} disabled={paragraphIndex === paragraphs.length - 1} onClick={() => {
