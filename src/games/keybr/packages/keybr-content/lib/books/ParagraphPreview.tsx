...
export const ParagraphPreview = memo(function ParagraphPreview({ paragraphs, paragraphIndex, around = 2, }: {
    ...
    return (<div class={styles.root}>
      {items.map(([index, paragraph]) => (<div class={clsx(styles.item, index === paragraphIndex
                ? styles.itemActive
                : styles.itemInactive)}>
...
export function ParagraphIndex({ paragraphIndex, }: {
    readonly paragraphIndex: number;
}) {
    return <span class={styles.index}>#{paragraphIndex + 1}</span>;
}
export function ParagraphContent({ paragraph, }: {
    readonly paragraph: string;
}) {
    return <span class={styles.content}>{paragraph}</span>;
}
