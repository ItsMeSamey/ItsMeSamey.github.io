import { type ComponentType, type FC, memo, type ReactNode, useEffect, useState, } from "@keybr/solid-compat/react";
type Unknown = unknown;
export const useDeferred = <T extends Unknown>(value: T): T => {
    const [deferred, setDeferred] = useState<T>(value);
    useEffect(() => {
        const id = requestAnimationFrame(() => {
            (setDeferred as any)(value);
        });
        return () => {
...
export const withDeferred = <P extends Record<string, Unknown>>(WrappedComponent: ComponentType<P>): FC<P> => {
