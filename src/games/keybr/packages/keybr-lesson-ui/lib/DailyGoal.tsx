...
export const DailyGoal = ({ id, className, dailyGoal, }: {
    id?: string;
    className?: ClassName;
    dailyGoal: DailyGoalType;
}) => {
    return (<span id={id} class={clsx(styles.root, className)}>
      <DailyGoalLabel value={dailyGoal.value} goal={dailyGoal.goal}/>
      <DailyGoalGauge value={dailyGoal.value}/>
    </span>);
};
const DailyGoalLabel = ({ value, goal }: {
    value: number;
    goal: number;
}) => {
    const { formatPercents } = useIntlNumbers();
    const { formatDuration } = useIntlDurations();
    return (<Value value={`${formatPercents(value, 0)}/${formatDuration({ minutes: goal })}`}/>);
};
const DailyGoalGauge = ({ value }: {
    value: number;
}) => {
