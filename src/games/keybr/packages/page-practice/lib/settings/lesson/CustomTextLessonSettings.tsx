...
export function CustomTextLessonSettings(solidProps: {
    ...
    return (<>
      <Explainer>
        <Description>
          <FormattedMessage id="lessonType.customText.description" defaultMessage="Generate typing lessons from the words of your own custom text. All keys are included by default. This mode is for the pros."/>
        </Description>
      </Explainer>
...
function CustomTextInput(): ReactNode {
    ...
    return (<>
      <Para>
        ...
        {exampleTexts.map(({ title, content }, index) => (<span>
            {index > 0 ? ", " : null}
            <LinkButton onClick={() => {
                updateSettings(settings.set(lessonProps.customText.content, content));
            }}>
              {title}
            </LinkButton>
          </span>))}
      ...
      <Para>
        <TextField type="textarea" placeholder={formatMessage({
            id: "t_Custom_text",
            defaultMessage: "Custom text",
        })} value={settings.get(lessonProps.customText.content)} onChange={(value) => {
    ...
    return (<FieldList>
      <Field>
        <CheckBox checked={settings.get(lessonProps.customText.lettersOnly)} label={formatMessage({
            ...
            updateSettings(settings.set(lessonProps.customText.lettersOnly, value));
        }}/>
      </Field>
      <Field>
        <CheckBox checked={settings.get(lessonProps.customText.lowercase)} label={formatMessage({
            ...
            updateSettings(settings.set(lessonProps.customText.lowercase, value));
        }}/>
      </Field>
      <Field>
        <CheckBox checked={settings.get(lessonProps.customText.randomize)} label={formatMessage({
            ...
            updateSettings(settings.set(lessonProps.customText.randomize, value));
        }}/>
      </Field>
    </FieldList>);
}
