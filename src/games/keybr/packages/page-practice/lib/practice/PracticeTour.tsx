...
export const PracticeTour = memo(function PracticeTour({ onClose, }: {
    readonly onClose?: () => void;
}) {
    return (<Tour onClose={onClose}>
      <Slide size="large">
      ...
      <Slide size="large">
        <FormattedMessage id="m_tour02" defaultMessage={"<p>Our teaching method is based on these principles:</p>" +
      ...
      <Slide size="large">
        <FormattedMessage id="m_tour03" defaultMessage={"<p>Initially it starts generating words from a small subset of the most frequent letters in the alphabet.</p>" +
      ...
      <Slide size="large">
        <FormattedMessage id="m_tour04" defaultMessage={"<p>When the algorithm includes a new letter to the current subset, the frequency of this letter is artificially increased so that it will be included in every generated word of a lesson.</p>" +
      ...
      <Slide size="small" anchor={`#${names.accuracy}`} position="block-end">
        <FormattedMessage id="m_tour08" defaultMessage={"<p>This is the accuracy indicator and the difference from the average value. Your goal is to increase this metric, meaning higher values are better.</p>" +
      ...
      <Slide size="small" anchor={`#${names.score}`} position="block-end">
        <FormattedMessage id="m_tour09" defaultMessage={"<p>This is the typing score indicator in abstract points and the difference from the average value.</p>" +
      ...
      <Slide size="small" anchor={`#${names.keySet}`} position="block-end">
        <FormattedMessage id="m_tour10" defaultMessage="<p>This indicator shows the current subset of letters used to generate the lessons, and your confidence level for every letter in the subset:</p>"/>
        <KeyLegendList />
      </Slide>
      <Slide size="small" anchor={`#${names.keySet}`} position="block-end">
        <FormattedMessage id="m_tour11" defaultMessage="<p>This indicator can also be used to predict the remaining number of lessons that are needed to fully unlock a letter, like in the example chart below. Visit it regularly to get more insight on how your learning is going, to see if you are making a progress.</p>"/>
        <KeyDetailsChartDemo />
      </Slide>
      <Slide size="small" anchor={`#${names.currentKey}`} position="block-end">
        <FormattedMessage id="m_tour12" defaultMessage={"<p>This indicator shows details about the letter with increased frequency, which is currently included in every generated word:</p>" +
            "<dl>" +
