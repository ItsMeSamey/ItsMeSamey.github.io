    ...
    function draw([shape, f]: Item, index: number): ReactNode {
        ...
        switch (solidProps.modifier) {
            case "h": {
                // Top left semicircle.
                const r = f * 15 + 5;
                return (<path class={clsx(styles.spot, styles.spotH)} d={`M ${x - r} ${y + r} A${r} ${r} 0 0 1 ${x + r} ${y - r}`}/>);
            }
            case "m": {
                // Bottom right semicircle.
                const r = f * 15 + 5;
                return (<path class={clsx(styles.spot, styles.spotM)} d={`M ${x - r} ${y + r} A ${r} ${r} 0 0 0 ${x + r} ${y - r}`}/>);
            }
            case "f": {
                // Full circle.
                const r = f * 20 + 5;
                return (<circle class={clsx(styles.spot, styles.spotF)} cx={x} cy={y} r={r}/>);
            }
        }
    }
});
