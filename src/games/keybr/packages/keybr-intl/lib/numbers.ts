import { type FormatNumberOptions, type IntlShape, useIntl } from "@keybr/solid-compat/intl";
...
const factory = (intl: IntlShape): IntlNumbers => {
    ...
    const formatNumber = (value: number, opts: number | FormatNumberOptions = formatNumberOpts): string => {
    ...
    const formatPercents = (value: number, opts: number | FormatNumberOptions = formatPercentsOpts): string => {
        if (typeof opts === "number") {
            opts = {
                style: "percent",
                maximumFractionDigits: opts,
